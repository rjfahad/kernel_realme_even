...
export KBUILD_COMPILER_STRING=$(./greenforce-clang/bin/clang -v 2>&1 | head -n 1 | sed 's/(https..*//' | sed 's/ version//')
export FW="RUI2"
...
export GREENFORCE_DIR="/home/rjfahad/RUI2_EVEN/greenforce-clang"
export GREENFORCE_REPO="https://github.com/greenforce-project/greenforce_clang.git"
...
clone_dependencies() {
    ...
    # Greenforce Clang
    # ===============================
    if [ ! -d "${PWD}/greenforce-clang" ]; then
        echo -e "${CYAN}Greenforce Clang not found. Cloning...${NOCOLOR}"
        git clone --depth=1 ${GREENFORCE_REPO} greenforce-clang || exit 1
        echo -e "${GREEN}Greenforce Clang cloned.${NOCOLOR}"
    else
        echo -e "${GREEN}Greenforce Clang already exists. Skipping clone.${NOCOLOR}"
    fi
