...
void cache_sid(void)
{
    ...
    extern void susfs_set_ksu_sid(u32 sid);
    extern u32 susfs_kernel_sid;
    u32 kernel_sid;
    if (cached_su_sid) {
        susfs_set_ksu_sid(cached_su_sid);
    }
    err = security_secctx_to_secid("u:r:kernel:s0", strlen("u:r:kernel:s0"),
                                   &kernel_sid);
    if (err) {
        pr_warn("Failed to cache kernel SID: %d\n", err);
        susfs_kernel_sid = 0;
    } else {
        susfs_kernel_sid = kernel_sid;
        pr_info("Cached kernel SID: %u\n", susfs_kernel_sid);
    }
#endif
}
